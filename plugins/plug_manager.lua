...
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مٌكِسِيَكِوَ ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil) end  else
if not plugin_exists(matches[2]) then 
return '🗂*¦* لا يوجد ملف بهذا الاسم .\n\n'
else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua",'🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مٌكِسِيَكِوَ ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil)
end end end
...
if not we_sudo(msg) then return "🍥￤هذا الاوامر للمطور الاساسي فقط 🛠" end
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./"..matches[2].."/"..matches[3], '🗂¦ آلمـلف مـقدم مـن قنآ‏‏هہ مٌكِسِيَكِوَ ¦ֆ \n📡¦ تابع قناة السورس @MEXIC_TV\n👨🏽‍🔧', dl_cb, nil) end
